...
import { SPRING_PALETTE } from '../constants';

const p = SPRING_PALETTE;

export const springFragmentShader = /* wgsl */`
${uniformsStruct}
${acesTonemapping}
${blockNoise}
...
const AMBIENT:  vec3f = vec3f(0.32, 0.38, 0.42);
...
fn main(input: FragIn) -> @location(0) vec4f {
  ...
  if (bt == 0u) {
    ...
    // ── Spring Leaves (QR dark modules / canopy) ─────────────
    // Deep forest green — linear brightness well below 0.20 for QR scan
    let leafLight = vec3f(0.12, 0.42, 0.08);
    let leafMid   = vec3f(0.08, 0.30, 0.05);
    let leafDeep  = vec3f(0.04, 0.20, 0.03);
    let leafRich  = vec3f(0.02, 0.12, 0.02);
    let t = n1;
    if (t < 0.33) {
    ...
    if (isTop) {
      // Top face transitions to spring leaf color in flat 2D view
      let leafLight = vec3f(0.12, 0.42, 0.08);
      let leafMid   = vec3f(0.08, 0.30, 0.05);
      var leafColor: vec3f;
      if (n1 < 0.5) {
        ...
        leafColor = mix(leafMid, vec3f(0.04, 0.20, 0.03), (n1 - 0.5) / 0.5);
      }
    ...
    // ── Outer Grass (bright spring green) ────────────────────
    let grassDark   = vec3f(0.06, 0.30, 0.06);
    let grassMid    = vec3f(0.10, 0.42, 0.10);
    let grassBright = vec3f(0.16, 0.58, 0.14);
    if (n1 < 0.5) {
    ...
    // ── Spring Ground (mossy / small flowers) ────────────────
    let mossLight = vec3f(0.32, 0.44, 0.22);
    let mossDark  = vec3f(0.22, 0.32, 0.14);
    let flowerYellow = vec3f(0.38, 0.38, 0.10);
    let isFlower = n3 > 0.75;
    if (isFlower) {
      albedo = mix(flowerYellow, mossDark, n1 * 0.6);
    } else {
      albedo = mix(mossLight, mossDark, n1);
    }
    albedo *= (0.9 + n2 * 0.15);
    if (isSide) { albedo *= 0.75; }
  }
